...
public class ShulkerGuiInsert extends Screen {
    ...
    public void GuiScreenEventInit(GuiScreenEvent.InitGuiEvent.Post event) {
        ...
        if (event.getGui() instanceof GuiShulkerBox) {
            event.getButtonList().add(new GuiButton(11209, event.getGui().width / 2 + 100, event.getGui().height / 2 - ((GuiShulkerBox) event.getGui()).getYSize() + 110, 50, 20, "Steal"));
            event.getButtonList().add(new GuiButton(11210, event.getGui().width / 2 + 100, event.getGui().height / 2 - ((GuiShulkerBox) event.getGui()).getYSize() + 130, 50, 20, "Store"));
            event.getButtonList().add(new GuiButton(11211, event.getGui().width / 2 + 100, event.getGui().height / 2 - ((GuiShulkerBox) event.getGui()).getYSize() + 150, 50, 20, "Drop"));
        }
    }
