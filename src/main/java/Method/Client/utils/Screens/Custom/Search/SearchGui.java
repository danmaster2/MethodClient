...
public final class SearchGui extends GuiScreen {
    ...
    public void initGui() {
        ...
        listGui = new ListGui(mc, this, SearchGuiSettings.getBlockNames(), 500, 0);

        Allblocks = new ListGui(mc, this, SearchGuiSettings.getAllBlockNames(SearchGuiSettings.getBlockNames()), 0, 0);
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        ...
        switch (button.id) {
            case 0:
                SearchGuiSettings.add(blockToAdd);
                blockNameField.setText("");
                Allblocks.list = SearchGuiSettings.getAllBlockNames(SearchGuiSettings.getBlockNames());
                break;

            case 1:
                SearchGuiSettings.remove(listGui.selected);
                break;
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException {
        ...
        if (!blockNameField.getText().isEmpty())
            Allblocks.list = SearchGuiSettings.SearchBlocksAll(SearchGuiSettings.getBlockNames(), blockNameField.getText());
        else
            Allblocks.list = SearchGuiSettings.getAllBlockNames(SearchGuiSettings.getBlockNames());
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(mc.fontRenderer, "Search" + " (" + listGui.getSize() + ")", width / 2, 12, 0xffffff);
    ...
    private static class ListGui extends GuiScrollingList {
        ...
        protected void drawSlot(int slotIdx, int entryRight, int slotTop, int slotBuffer, Tessellator tess) {
            ...
            fr.drawString(displayName + " (" + name + ")", 68 + offsetx, slotTop + 2,
                    0xf0f0f0);
        }
